...
public class ProjectController {
    ...
    @ResponseBody
    @GetMapping("/project")
    public AjaxResponse viewProjectListPage() {
        ProjectListVO projectListVO = projectService.getAllProject();

        return new AjaxResponse().append("project", projectListVO);
    }
    ...
    // 작성자 추가를 위해 SessionAttribute 추가 필요, @SessionAttribute("_LOGIN_USER_") MemberVO memberVO
    // form action 추가 필요
    @PostMapping("/project/write")
    public String writeProject(CreateProjectVO registProjectVO) {
        // 0. session memberVO가 admin 이 아닌 경우, return list page or return 400 page(잘못된 접근)
    ...
    public String modifyProject(@PathVariable String projectId) {
        ...
        // 2. 세션으로 관리자 판별 (originalProjectVO와 유저를 판별 및 유저 권한으로 판별), 실패 시 throw new RuntimeException
    ...
    public String deleteProject(@PathVariable String projectId) {
        ...
        // 2. 검증 로직 (originalProjectVO와 유저를 판별 및 유저 권한으로 판별), 실패 시 throw new RuntimeException
