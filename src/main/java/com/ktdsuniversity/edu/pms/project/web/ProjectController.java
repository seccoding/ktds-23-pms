...
public class ProjectController {
	...
	public String viewProjectDetailPage(@RequestParam String prjId,
		...
		int projectTeammateCount = projectService
				.getProjectTeammateCount(prjId);
		List<RequirementVO> projectRequirementsList = requirementService
				.getAllRequirement(prjId).stream()
		...
		boolean isTeammate = projectVO.getProjectTeammateList().stream()
				.anyMatch(teammate -> teammate.getTmId()
						.equals(employeeVO.getEmpId()));
		...
		// PM 뽑기
		Optional<ProjectTeammateVO> pmOptional = projectVO
				.getProjectTeammateList().stream()
				.filter(projectTeammateVO -> projectTeammateVO.getRole()
						.equals("PM"))
				.findFirst();
	...
	public AjaxResponse responseProjectStatus(@PathVariable String projectId) {
		return new AjaxResponse().append("chartData",
				projectService.getProjectStatus(projectId));
	}
	...
	public String viewProjectTeamPage(@RequestParam String prjId, Model model,
		...
		List<ProjectTeammateVO> teammate = projectService
				.getAllProjectTeammateByProjectId(prjId);
	...
	public String viewProjectWritePage(
		...
		List<EmployeeVO> employeeList = employeeService.getAllEmployee()
				.getEmployeeList();
		List<DepartmentVO> departmentList = departmentService
				.getOnlyDepartment().getDepartmentList();
	...
	public AjaxResponse writeProject(CreateProjectVO createProjectVO,
		...
		validator.add("prjName", Type.NOT_EMPTY, "프로젝트명을 입력해주세요.")
				...
				.add("strtDt", Type.DATE, createProjectVO.getEndDt(),
		...
		return new AjaxResponse().append("next",
				"/project/view?prjId=" + prjId);
	}

	@GetMapping("/project/modify/{prjId}")
	public String viewProjectModifyPage(@PathVariable String prjId, Model model,
			@SessionAttribute("_LOGIN_USER_") EmployeeVO employeeVO) {
	...
	public AjaxResponse modifyProject(@PathVariable String prjId,
		...
		validator.add("prjName", Type.NOT_EMPTY, "프로젝트명을 입력해주세요.")
				...
				.add("strtDt", Type.DATE, modifyProjectVO.getEndDt(),
						"종료일은 시작일보다 이후여야 합니다. 날짜를 다시 설정해주세요")
				.start();
		...
		// 4. 수정시작
		boolean isUpdateSuccess = projectService
				.updateOneProject(modifyProjectVO);
	...
	public AjaxResponse deleteMassiveTeammate(
			@RequestParam("deleteItems[]") List<String> deleteItems,
			@SessionAttribute("_LOGIN_USER_") EmployeeVO employeeVO) {
	...
	public AjaxResponse viewDepartmentTeammate(@PathVariable String deptId,
			@SessionAttribute("_LOGIN_USER_") EmployeeVO employeeVO) {
		List<EmployeeVO> allEmployeeList = employeeService.getAllEmployee()
				.getEmployeeList();

		List<EmployeeVO> list = allEmployeeList.stream()
				.filter(emp -> emp.getDeptId().equals(deptId)).toList();
	...
	public AjaxResponse addNewProjectTeammate(
			ProjectTeammateVO newProjectTeammate,
			@SessionAttribute("_LOGIN_USER_") EmployeeVO employeeVO) {
		ProjectVO projectVO = projectService
				.getOneProject(newProjectTeammate.getPrjId());
		...
		boolean addResult = projectService
				.insertOneTeammate(newProjectTeammate);

		return new AjaxResponse().append("result", addResult).append("message",
				"이미 존재하는 팀원입니다.");
	}
}
